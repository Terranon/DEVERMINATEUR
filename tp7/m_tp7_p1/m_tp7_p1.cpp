 ...
 * Name:        m_tp7_p1.cpp
 * Description: When button is pressed, counter increments by 10 per second.
 *              When button is released or when counter reaches 120, LED will 
 *              flash green for .5 seconds.
 *              Two seconds later LED will flash red (counter/2)times
 *              at a rate of 2 flashes per second.
 *              Then, LED will light up green for one second.
 *              Finally, LED will return to initial state.
 * 
 *              Pinout :    LED ( + - )
 *                                | |
 ...
 */
void startCounter0A (uint32_t duration) {

    // value of Timer/Counter 1, 16 bits
    TCNT1 = 0;
    
    // Output Compare Register 1 A
    OCR0A = (duration * (F_CPU >> 10)) / 1000;
    
}

void startCounter0B (uint32_t duration) {
...
void startCounter1A (uint32_t duration) {
...
void flashGreenHalfSecond() {
    
    timerNotComplete = 1;
    
    while (timerNotComplete) {
        PORTC = GREEN;
        _delay_loop_ms (FLICKER_TIME);
        PORTC = OFF;
        _delay_loop_ms (FLICKER_TIME;
    }
}

void flashRedCounterRatioTimes() {
    
    for (uint8_t i = 0; i < (counter/2); i++) {
        PORTC = RED;
        _delay_loop_ms (HALF_SECOND);
    }
void lightUpGreen() {
    PORTC = GREEN;
    _delay_loop_ms (2*HALF_SECOND);
...
ISR (TIMER0_COMPA_vect) {
    //TODO: change state when counter reaches 120
}
...
ISR (TIMER1_COMPA_vect) {
    //increases counter every .1 seconds
    counter++;
}
