 ...
 * Name:        j_tp7_p1.cpp
 * Description: Flash the LED with an external button
 * 
 * Pinout :                              LED (+ -)
 *                                            | |                          
...
int main () {
    ...
    while (1) {
        ...
        // Set the timer to interrupt every 100 ms
        startTimer(100);
    
        // Wait for the button to be released, or the counter to reach 120 (12 secs)
        while (buttonPressed && buttonCounter < 120) {
        ...
        // Light the LED green for 500 ms
        ...
        // Flash the LED red (buttonCounter/2) times, at 2 Hz
        for (uint8_t i = 0; i < (buttonCounter >> 1); i++) {
        ...
        // Light the LED green for 1000 ms
        led.setColor(Led::GREEN);
        ...
        // Turn off the LED before resetting
        led.setColor(Led::OFF);
