...
enum State {INIT, START, MUR_GAUCHE, MUR_DROIT, ROTATION_GAUCHE, ROTATION_DROIT, TRANSITION_GAUCHE, TRANSITION_DROIT};
...
\******************************************************************************/
void loop(){
	Sensor sensor(2, 0);			//on initialise le sensor gauche a PA0 et le sensor droit a PA2
	...
	while(1){
		...
		switch(etat){
			...
			case INIT:								//le robot ne commence pas avant d'avoir reussi l'epreuve initiale
				if(distanceDetecteurDroit < distanceDetecteurGauche){
					PORTC = 0x01;
					if(distanceDetecteurGauche < distanceDetecteurDroit){
						PORTC = 0x02;
						etat = START;
						_delay_ms(2000);
						PORTC = 0x00;
		...
		uint8_t detectionObstacle = detectionObstacle();
